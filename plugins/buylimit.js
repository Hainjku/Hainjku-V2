...
let handler = async (m, { conn, command, args }) => {
  ...
  if (global.DATABASE._data.users[m.sender].exp >= xpperlimit * count) {
    ...
    conn.reply(m.chat, `💰 *BANCO*\n\nHas comprado coins al banco a la compra actual del coin.\n\n*Gastado:* -${xpperlimit * count} XP\n*Comprado:* +${count} coin(s)`, m)
  } else conn.reply(m.chat, `💰 *BANCO*\n\nError de transacción! No tienes suficiente XP como para comprar ${count} coin(s).`, m)
}
