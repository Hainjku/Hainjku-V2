...
let handler = async (m, { conn, text, usedPrefix }) => {
    if (text) {
        try {
            let res = await fetch('https://api.zeks.xyz/api/watercolor2?text=' + encodeURIComponent(text) + '&apikey=apivinz')
    ...
    } else throw `contoh: \n\n${usedPrefix}watercolor ariffb`
}
handler.help = ['watercolor <teks>']
